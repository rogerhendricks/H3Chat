...
app = FastAPI(
    ...
    description="RAG Pipeline for querying pacemaker and defibrillator technical manuals.",
    version="1.0.0",
...
def _set_progress(job_id: str, status: str, percent: float, message: str = "", extra: dict | None = None):
    payload = {
        "status": status,
        "percent": percent,
        "message": message,
...
def _run_ingest_job(job_id: str, temp_path: str, title: str, manufacturer: str, device_model: str, doc_type: str, year: int):
    def progress(stage, payload):
        if stage == "start":
            _set_progress(job_id, "running", 5, "Upload received. Starting extraction...")
        elif stage == "markdown_extracted":
            _set_progress(job_id, "running", 25, "Markdown extracted from PDF.")
        elif stage == "chunked":
            total = payload.get("total_chunks", 0)
            _set_progress(job_id, "running", 40, f"Chunked document into {total} parts.", {"total_chunks": total})
        elif stage == "chunk_progress":
            current = payload.get("current", 0)
            total = payload.get("total", 1)
            percent = 40 + (50 * (current / total))
            _set_progress(job_id, "running", min(percent, 95), f"Embedding chunks ({current}/{total})...", {"current": current, "total": total})
        elif stage == "completed":
            _set_progress(job_id, "completed", 100, "Ingestion complete.", payload)
        elif stage == "error":
            _set_progress(job_id, "error", 100, f"Ingestion failed: {payload.get('message','')}")
...
async def ask_documents(request: SearchRequest):
    ...
    try:
        ...
        retrieved_chunks = []
        for row in rows:
            retrieved_chunks.append(SearchResult(
                title=row[0], manufacturer=row[1], device_model=row[2],
                content=row[3], page_number=row[4], similarity=float(row[5])
            ))
        ...
        if not retrieved_chunks:
            return GenerationResponse(answer="No relevant documents found in the database.", sources=[])
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                timeout=httpx.Timeout(300.0, connect=10.0) # Allow slower local-model inference while failing fast on bad connectivity
            )
